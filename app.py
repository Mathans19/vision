...
def mark_attendance():
    # Set current time in IST at the beginning of the function
    current_datetime_ist = datetime.now(IST)
    
    data = request.json
    student_id = data['student_id']
    student_name = data['student_name']
    current_date = current_datetime_ist.date()  # Use IST for current date
        ...
        for i, period in enumerate(periods, start=1):
            if current_time >= period["start_time"] and current_time <= period["end_time"]:
                ...
                def add_header(response):
                    response.headers['Date'] = current_datetime_ist.strftime('%a, %d %b %Y %H:%M:%S %Z')
                    return response
                return response, 200
